...
function sendBadgeApplication(args, callback) {
  ...
  function loadUserParent(profile, done) {
    if(_.contains(earner.types, 'attendee-u13')) {
      if(profile.parents.length > 0) {
        seneca.act({role: 'cd-profiles', cmd: 'list_query', query: {userId: profile.parents[0]}}, function (err, parentProfiles) {
          if(err) return done(err);
  ...
  function sendEarnerEmail(profile, parentProfile, done) {
    ...
    if(!_.isEmpty(parentProfile)) {
      ...
      emailCode = 'accept-badge-award-for-ninja-u13-' + locality;
    } else {
      recipientEmail = profile.email;
      emailCode = 'accept-badge-award-' + locality;
    }
