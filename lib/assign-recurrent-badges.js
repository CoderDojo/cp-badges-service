...
function assignRecurrentBadges (args, callback) {
  ...
  var recurrentBadges = [{slug: 'my-1st-dojo!', recurrence: 1, only: ['attendee-u13', 'attendee-o13']},
  ...
  function checkAvailableBadges (dojoUser, mozillaBadges, done) {
    var badges = [];
    seneca.act({role: 'cd-events', cmd: 'searchApplications', query: {userId: application.userId, 'attendance': {ne$: '{}'}}}, function (err, applications) {
      if (err) return done(err);

      badges = _.filter(recurrentBadges, function (badge) {
        return filterRecurrent(applications.length, badge.recurrence) && filterUserType(dojoUser.userType, badge.only);
      });
  ...
  function awardBadges (dojoUser, badges, done) {
    if (badges.length > 0) {
      async.each(badges, function (badge, callback) {
        ...
        applicationData: applicationData, user: {id: null},
