...
function exportBadges (args, callback) {
  ...
  function loadUsersBadges (done) {
    seneca.act({role: 'cd-profiles', cmd: 'list', query: {userId: user.id}}, (err, response) => {
  ...
  function bakeBadgeImages (badgeData, done) {
    const bakedBadges = [];
    async.eachSeries(badgeData, (badge, cb) => {
  ...
  const download = (uri, filename, callback) => {
    request.get({uri: uri, encoding: null}, (err, res, body) => {
