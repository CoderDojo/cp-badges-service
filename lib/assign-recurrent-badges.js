...
function assignRecurrentBadges(args, callback) {
  ...
  const recurrentBadges = [
    ...
    {
      slug: 'europe-code-week-2016',
      recurrence: 1,
      dates: ['2016-10-15 00:00:00', '2016-10-31 23:59:59'],
      evidence: `Participed in Code Week 2016 by attending a CoderDojo event on the ${moment
        .utc(args.application.attendance[0])
        .format('DD/MM/YYYY')}`,
    },
  ...
  async.waterfall([getProfile, getBadges, checkAvailableBadges, awardBadges], (err, res) => {
  ...
  function getBadges(dojoUser, done) {
    ...
    seneca.act({ role: plugin, cmd: 'listBadges' }, (err, response) => {
      ...
      mozillaRecurrentBadges = _.filter(response.badges, b =>
        _.find(recurrentBadges, { slug: b.slug }));
  ...
  function checkAvailableBadges(dojoUser, mozillaBadges, done) {
    let badges = [];
    const filterRecurrent = (value, reference) => value >= reference;
    const filterUserType = (value, references) => _.intersection(references, value).length > 0;
    seneca.act({
      role: 'cd-events',
      ...
      badges = _.filter(recurrentBadges, (badge) => {
        ...
        return (
          filterRecurrent(applications.length, badge.recurrence) && acceptedUserType && inDates
        );
      });
      ...
      const newBadges = _.filter(
        awardableBadges,
        badge => !_.find(dojoUser.badges, { id: badge.id }),
      );
  ...
  function awardBadges(dojoUser, badges, done) {
    if (badges.length > 0) {
      async.eachSeries(badges, (badge, cb) => {
        ...
        seneca.act({
          role: plugin,
          cmd: 'sendBadgeApplication',
          applicationData: _.clone(applicationData),
          user: { id: null },
          zenHostname,
        }, cb);
      }, (err) => {
